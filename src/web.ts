...
import type { SensorsPlugin, SensorOptions, Sensor, WebPermissionStatus } from './definitions';
...
const getWindowProperty = (type: WebSensorType) => Object.keys(webSupportedSensors).find(key => webSupportedSensors[key] === type)

export class SensorWeb implements Sensor {
  private sensor?: Record<string, any> = undefined
...
export class SensorsWeb extends WebPlugin implements SensorsPlugin {
  async requestPermissions(sensor: Sensor): Promise<WebPermissionStatus[]> {
